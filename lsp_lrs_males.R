...
# MODEL: HERITABILITY OF LIFESPAN and LRS for MALE RECRUITS
...
setwd("/srv/home/abriloeb/wp2/h2/males")
...
bfb_males <- readRDS("bfb_males.rds")
...
# variable of interest: lifespan and LRS
# fixed effects: none
# random effects: animal, cohort and nest

# Selecting only the relevant columns for the dataframe

library(tidyverse)

bfb_males <- select(bfb_males, animal, sire, dam, nest, cohort, lifespan, lrs.fledged)
...
#1.-lsp_lrs_recruits_poisson

# Setting up the prior: In this case I will be using a PARAMETER-EXPANDED PRIOR
# As suggested by de Villemereuil (as it it more gentle for variances close to zero)

prior_lsp_lrs<- list(R = list(V = diag(2), nu = 2),
...
lsp_lrs_males <- MCMCglmm(cbind(lifespan, lrs.fledged) ~ trait - 1,
...
saveRDS(lsp_lrs_males,
        file = paste("lsp_lrs_males_", args, ".rds", sep=""))
